...
import by.it.karpeichyk.Calc.CalcExeption;
...
class Parser {
    ...
    private String oneOperationCalc(String left, String operation, String right) throws CalcExeption {
        //получаем вторую переменную (в методе createVar нужно предусмотреть чтение значения из карты)
        Var two = Var.createVar(right);
        if (two == null)
            throw new CalcExeption(String.format(" Ошибка %s%s%s:", left, operation, right));
        ...
        throw new CalcExeption(" Неизвестная Ошибка ");
    }
    ...
    String calc(String expression) throws CalcExeption {
        String res = null;
